...
def mkslide_gui():
    ...
    st.markdown("""画像情報が記載されたエクセルファイルをアップロードし「MKSLIDEが作成したCSVファイルですか？」を
    NoにしてMKSLIDE用のCSVファイルを作成してください。問題なく作成されると表が出力されますので、表の右上のメニューアイコンから
    ダウンロードしてください（これはエクセルインジェクションを防ぐための手順です）。次に、ダウンロードしたCSVファイルをアップロードして
    「MKSLIDEが作成したCSVファイルですか？」をYesにして画像をアップロードしてください。条件を選択すれば、条件に該当するファイルが
    抽出され表示されます。出力結果はパワーポイントのスライドとしてダウンロード可能です。テンプレートをアップロードして、
    好みのスライドデザインに表示画像を出力することも可能です。""") 
    st.markdown("---")
