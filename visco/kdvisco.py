...
def kdvisco_gui():
    ...
    #st.markdown("""$$\left[ \eta \right]$$: （粉体を一切含まない基材の粘度）""")
    st.markdown(r"$\left[ \eta \right]$: （粉体を一切含まない基材の粘度）")
    st.markdown("""$$\phi_{\mathrm{max}}$$: 最大充填体積分率（粘度を決める最も重要なパラメータ。離散要素法や実験から決める。）""")
    st.markdown("---")
